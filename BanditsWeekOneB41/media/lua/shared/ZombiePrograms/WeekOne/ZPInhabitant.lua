...
ZombiePrograms.Inhabitant.Main = function(bandit)
    ...
    local dist2B = 1000
    local home2Coords
    local brain = BanditBrain.Get(bandit); print(brain.weapons.melee)
    local menuRecipePicked = "NOPE"
    ...
    if foundWorkplace == 1 and dist2B < 200 then
        dist2B = BanditUtils.DistTo(bx, by, home2Coords.x, home2Coords.y)
        ...
        if brain.loot[1] == menuRecipePicked then
            ...
            local task = {action="TimeItem", anim=anim, sound=sound, item=item, left=true, time=20}
